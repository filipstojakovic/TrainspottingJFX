...
public class StreetVehicleSpawner extends Thread
{
    ...
    public static final String MAX_VEHICLE_NUM = "max_vehicles_num";
    ...
    public StreetVehicleSpawner(List<StreetRoad> streetRoads) throws IOException
    {
        ...
        File file = new File("C:\\Users\\filip\\IdeaProjects\\TrainspottingJFX\\src\\main\\resources\\street_vehicles.config");
            ...
            try
            {
                ...
                while (true)
                {
                    ...
                    for (WatchEvent<?> event : key.pollEvents())
                    {
                        ...
                        if (fileName.toString().trim().endsWith(".config") && kind.equals(ENTRY_MODIFY))
                        {
    ...
    private synchronized void readStreetVehiclePropertyFile(File file) throws IOException
    {
        try (InputStream inputStream = new FileInputStream(file))
        {
            ...
            spawnSpeed = Integer.parseInt(properties.getProperty(SPAWN_SPEED));
            int maxVehicles = Integer.parseInt(properties.getProperty(MAX_VEHICLE_NUM));
